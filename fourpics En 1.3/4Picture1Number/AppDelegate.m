...
static NSString *const kTrackingId = @"UA-42848272-1";

@implementation AppDelegate

@synthesize databaseName, databasePathNew;
...
+ (void)initialize
{
    ...
    [iRate sharedInstance].applicationBundleID = @"com.eurisko.fourpics";
    [iRate sharedInstance].appStoreID = 642297180;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    ![[NSUserDefaults standardUserDefaults] boolForKey:kTrackingId];
    ...
    if (![userDefault objectForKey:@"FirstRun"] || ![SimpleKeychain load:@"ScoreEn"]) {
        // Delete values from keychain here
        [SimpleKeychain delete:@"HostEn"];
        [SimpleKeychain delete:@"HostImageLink"];
        [SimpleKeychain delete:@"ScoreEn"];
        [SimpleKeychain delete:@"levelEn"];
        [SimpleKeychain delete:@"LivesEn"];
        [SimpleKeychain delete:@"Question"];
        [SimpleKeychain delete:@"deviceUDID"];
        [SimpleKeychain delete:@"questionlevelidEn"];
        [SimpleKeychain delete:@"isLevelLoadedEn"];
        [SimpleKeychain delete:@"LastDate"];
        [SimpleKeychain delete:@"rewardsDate"];
        [SimpleKeychain delete:@"rewardsEndDate"];
        
        [SimpleKeychain delete:@"shareDateEn"];
        [SimpleKeychain delete:@"inviteDate"];
        [SimpleKeychain delete:@"isAppRatedEn"];
        [SimpleKeychain delete:@"facebookID"];
        [SimpleKeychain delete:@"UpdateRewardEnglish"];
        ...
        if(SYSTEM_VERSION_LESS_THAN(@"6.0")) {
            ...
            [SimpleKeychain save:@"deviceUDID" data:[[[UIDevice currentDevice] identifierForVendor] UUIDString]];
        }
        
        [SimpleKeychain save:@"ScoreEn" data:[Cryptography TripleDES:@"400" algo:kCCEncrypt key:@"Sfg$93@B"]];
        [SimpleKeychain save:@"LivesEn" data:[Cryptography TripleDES:@"5" algo:kCCEncrypt key:@"Sfg$93@B"]];

        [SimpleKeychain save:@"levelEn" data:@"1"];
        [SimpleKeychain save:@"Question" data:[[NSString alloc] initWithFormat:@"%d", 0]];
        [SimpleKeychain save:@"questionlevelidEn" data:[[NSString alloc] initWithFormat:@"%d", 13]];
        [SimpleKeychain save:@"isLevelLoadedEn" data:[[NSString alloc] initWithFormat:@"%d", 0]];
        self.SoundEnabled = YES;
        
        [userDefault setValue:@"questionsNew.sqlite" forKey:@"databaseName"];
    ...
    if([SimpleKeychain load:@"HostEn"] == NULL){
        // Add the host link and image link static
        [SimpleKeychain save:@"HostEn" data:@"aHR0cHM6Ly93d3cuZXVyaXNrb21vYmlsaXR5Lm1lL2ZvdXJwaWNzb25lbnVtYmVyLw=="];
        [SimpleKeychain save:@"HostImageLink" data:@"aHR0cHM6Ly93d3cuZXVyaXNrb21vYmlsaXR5Lm1lL2ZvdXJwaWNzb25lbnVtYmVyLw=="];
    }
    
    self.hostName = [[NSString alloc] initWithData:[[NSData alloc] initWithBase64EncodedString:[SimpleKeychain load:@"HostEn"]] encoding:NSUTF8StringEncoding];
    self.imageURL = [[NSString alloc] initWithData:[[NSData alloc] initWithBase64EncodedString:[SimpleKeychain load:@"HostImageLink"]] encoding:NSUTF8StringEncoding];
    counterLabel.font = [UIFont fontWithName:@"TodaySHOP-Bold" size:70];

    self.databaseName = [[NSUserDefaults standardUserDefaults] objectForKey:@"databaseName"];
    
    if(self.databaseName == NULL) {
        self.databaseName = @"questions.sqlite";
        [userDefault setValue:@"questions.sqlite" forKey:@"databaseName"];
    }
    ...
    if(([self.databaseName isEqualToString:@"questions.sqlite"])){
        databasePathNew =[documentsDir stringByAppendingPathComponent:@"questionsNew.sqlite"];
        [self checkAndCreateDatabaseNew];
    }

    
    self.userLevel = [[SimpleKeychain load:@"levelEn"] intValue];
    if(self.userLevel == 0)
        self.userLevel = 1;
    
    self.score = [[Cryptography TripleDES:[SimpleKeychain load:@"ScoreEn"] algo:kCCDecrypt key:@"Sfg$93@B"] intValue];

    self.Lives = [[Cryptography TripleDES:[SimpleKeychain load:@"LivesEn"] algo:kCCDecrypt key:@"Sfg$93@B"] intValue];
    self.questionlevelid = [[SimpleKeychain load:@"questionlevelidEn"] intValue];
    self.isLevelLoaded = [[SimpleKeychain load:@"isLevelLoadedEn"] intValue];
    ...
    if (self.userLevel >= 13)
        rootView.ShoudReload = 1;

    if([self.databaseName isEqualToString:@"questions.sqlite"]){

        if(self.questionlevelid < 13) {
            
            rootView.ShoudReload = 0;
            
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applyupdates) name:@"applyupdates" object:nil];
            // fill the old database till level 12
            [DATABASE_HELPER add_questions_till_level12:self.questionlevelid];
    ...
    self.transparentGrayView.frame = CGRectMake(0, 0, self.window.frame.size.width, self.window.frame.size.height+20);
    
    self.congratulationsView.center = CGPointMake(self.window.bounds.size.width/2, self.window.bounds.size.height/2);
    ...
    [[Harpy sharedInstance] setAppID:@"642297180"];
    ...
    self.adView.frame = CGRectMake(0, 0, 320, self.window.frame.size.height);
...
- (void) applyupdates {
    ...
    if(![SimpleKeychain load:@"902updated"]){
        [DATABASE_HELPER update_question902_hint:@"QW5zd2VyIFN0YXJ0cyBieSAxMg=="];
        [SimpleKeychain save:@"902updated" data:@"done"];
    }
    
    if(![SimpleKeychain load:@"UpdateRewardEnglish"]){
...
- (void) checkforversion {
    ...
    if([version isEqualToString:@"1.1"]){
        ...
        l2.text = [[NSString alloc] initWithFormat:@"+100"];
        ...
        [SimpleKeychain save:@"ScoreEn" data:[Cryptography TripleDES:[[NSString alloc] initWithFormat:@"%d", self.score] algo:kCCEncrypt key:@"Sfg$93@B"]];
        ...
        [SimpleKeychain save:@"UpdateRewardEnglish" data:@"gotRewards"];
    }
}

- (void) addLives :(NSNotification *)notification {
    [SimpleKeychain save:@"LivesEn" data:[Cryptography TripleDES:[[NSString alloc] initWithFormat:@"%d", self.livesCycle.lives] algo:kCCEncrypt key:@"Sfg$93@B"]];
...
- (void) callToGetTimeLink  {
    ...
    NSString *path= [[NSString alloc] initWithFormat:@"https://www.euriskomobility.me/fourpicsonenumber/getDomain.php"];
...
- (void) parseTimeXMLData:(NSString *)XmlString {
    ...
    @try {
        ...
        for (CXMLElement *node in nodes) {
            ...
            [SimpleKeychain save:@"HostEn" data:[category objectForKey:@"hosting"]];
            [SimpleKeychain save:@"HostImageLink" data:[category objectForKey:@"imageURL"]];
...
- (void)application:(UIApplication *)app didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken {
    ...
    if([self.hostActive isEqualToString:@"down"])
    {
        ...
        [pushClass SaveTokenForPushNotification:token andAppID:@"99"];
    }
}
        ...
        for (CXMLElement *node in nodes) {
            ...
            if([Utils date:phoneDate isBetweenDate:self.rewardsDate andDate:self.rewardsEndDate]){
                ...
                if(![self.rewardsDate isEqualToString:[SimpleKeychain load:@"rewardsDate"]] && ![self.rewardsEndDate isEqualToString:[SimpleKeychain load:@"rewardsEndDate"]]){
                    ...
                    l1.text = @"YOU   WON";
                    l2.font = [UIFont fontWithName:@"TodaySHOP-Bold" size:20];
                    l2.text = [[NSString alloc] initWithFormat:@"+%@", self.rewardsVal];
                    l3.font = [UIFont fontWithName:@"TodaySHOP-Bold" size:20];
                    l3.text = @"COINS";
...
//    NSString *XMLData = [[NSString alloc] initWithBytes: [webData mutableBytes] length:[webData length] encoding:NSASCIIStringEncoding];
    ...
    NSString *XMLData = [[NSString alloc] initWithData:webData encoding:NSUTF8StringEncoding];

    if([XMLData isEqualToString:@"1"] || [XMLData isEqualToString:@"0"] || [XMLData length] == 0 || [XMLData length] == 1 || [XMLData isEqualToString:@" "]){
...
-(void) checkAndCreateDatabaseNew {
	BOOL success=0;
	NSFileManager *fileManager = [NSFileManager defaultManager];
	success = [fileManager fileExistsAtPath:databasePathNew];

	if(success)
		return;
    
	NSString *databasePathFromApp = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"questionsNew.sqlite"];
    
	[fileManager copyItemAtPath:databasePathFromApp toPath:databasePathNew error:nil];
}
...
- (void) startLifeTimerWith :(NSNotification *)notification {
    ...
    counterLabel.text = [[NSString alloc] initWithFormat:@"%d : %d", minutes, secondes];
...
- (void) ShowTimerView {
    ...
    TimerL4.text = [[NSString alloc] initWithFormat:@"%d", self.Lives];
    ...
    if(self.Lives >= 5) {
        ...
        TimerL1.text = @"TIME FOR NEXT LIFE.";
        ...
        if([phoneDate isEqualToString:[SimpleKeychain load:@"shareDateEn"]]) {
            ...
            TimerL3.text = @"Share on Facebook: +1 ♥";
        }
    }
...
- (IBAction)BuyLives:(id)sender {
    if ([SKPaymentQueue canMakePayments]) {
        ...
        purchaseManager.productID = @"com.eurisko.fourpics.lives";
...
-(void) updateLabel {
    ...
    if(minutes < 10 && secondes > 10)
        counterLabel.text = [[NSString alloc] initWithFormat:@"0%d : %d", minutes, secondes];
    else if(secondes < 10 && minutes > 10)
        counterLabel.text = [[NSString alloc] initWithFormat:@"%d : 0%d", minutes, secondes];
    else if(minutes < 10 && secondes < 10)
        counterLabel.text = [[NSString alloc] initWithFormat:@"0%d : 0%d", minutes, secondes];
    else
        counterLabel.text = [[NSString alloc] initWithFormat:@"%d : %d", minutes, secondes];
}

-(void)countDown {
    if(minutes == 30 && secondes>1)
        minutes = minutes - 1;
    ...
    if (secondes == 0 && minutes != 0) {
        ...
        [SimpleKeychain save:@"LivesEn" data:[Cryptography TripleDES:[[NSString alloc] initWithFormat:@"%d", self.Lives] algo:kCCEncrypt key:@"Sfg$93@B"]];
        NSString *phoneDate = [Utils dateTransform:[Utils getUTCFormateDate:[NSDate date]] FromFormat:@"yyyy-MM-dd HH:mm:ss" ToFormat:@"yyyy-MM-dd"];

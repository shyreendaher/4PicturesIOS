...
- (void)viewDidLoad
{
    ...
    if ([appDelegate.internetActive isEqualToString:@"down"] && appDelegate.userLevel >= 13)
    {
...
//    }
    ...
    self.transparentGrayView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height+20);
//    self.transparentGrayView.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height/2);
    ...
    self.buycoinsView.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height/2);
    self.ConfirmBuyView.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height/2);
    self.NotEnoughLifeView.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height/2);
    self.WrongAnswerView.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height/2);
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    levelLabel.text = [[NSString alloc] initWithFormat:@"Level %d", self.selectedLevel];
...
- (void) FillView {
    ...
    self.movieName = [[NSString alloc] initWithFormat:@"%@", [Utils CapitalWordOF:movie.QAnswer]];
...
- (IBAction) procceed_buy :(id)sender {
    ...
    if(NumberOfPurchasedFeature == 1){
        ...
        appDelegate.score = [[Cryptography TripleDES:[SimpleKeychain load:@"ScoreEn"] algo:kCCDecrypt key:@"Sfg$93@B"] intValue];
...
- (void) fill_characters {
    ...
    [letterb1 setTitle:@"1" forState:UIControlStateNormal];
    [letterb2 setTitle:@"2" forState:UIControlStateNormal];
    [letterb3 setTitle:@"3" forState:UIControlStateNormal];
    [letterb4 setTitle:@"4" forState:UIControlStateNormal];
    [letterb5 setTitle:@"5" forState:UIControlStateNormal];
    [letterb6 setTitle:@"6" forState:UIControlStateNormal];
    [letterb7 setTitle:@"7" forState:UIControlStateNormal];
    [letterb8 setTitle:@"8" forState:UIControlStateNormal];
    [letterb9 setTitle:@"9" forState:UIControlStateNormal];
    [letterb10 setTitle:@":" forState:UIControlStateNormal];
    [letterb11 setTitle:@"." forState:UIControlStateNormal];
    [letterb12 setTitle:@"0" forState:UIControlStateNormal];
        ...
        if([movieName length] == answerLetterIndex) {
            ...
            if([self.movieGuessName isEqualToString:self.movieName]){
                ...
                if(!movie.IsAnswered){
                    ...
                    l2.text = [[NSString alloc] initWithFormat:@"+ %d", 5];
...
- (IBAction) DeleteAll:(id)sender{
    ...
    tempallNumbers = allNumbers;
    self.movieName = [[NSString alloc] initWithFormat:@"%@", [Utils CapitalWordOF:movieName]];
    [self fill_empty_characters];
    self.movieName = [[[NSString alloc] initWithFormat:@"%@", [Utils CapitalWordOF:movieName]] stringByReplacingOccurrencesOfString:@" " withString:@""];
    [self performSelector:@selector(fill_characters) withObject:nil afterDelay:0.5];
}

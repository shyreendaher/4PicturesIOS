...
- (void) gotoMovie:(id)sender {
    ...
    if ([appDelegate.currentLevelQuestionArray count] != 0) {
        ...
        instance.selectedMovie = (int)[sender tag];
        [self.navigationController pushViewController:instance animated:YES];
        [instance release];
    }
}
...
- (void) CheckIfLevelCompleted:(NSNotification*)notification{
    ...
    if(AnsweredMoviesCount == 10) {
        LevelComplete = YES;
        if(self.selectedLevel == appDelegate.userLevel){
            appDelegate.userLevel ++;
            [SimpleKeychain save:@"level" data:[[NSString alloc] initWithFormat:@"%d", appDelegate.userLevel]];
...
- (void) levelComplete {
    ...
    l1.font = [UIFont fontWithName:@"TodaySHOP-Bold" size:25];
    l1.text = @"لقد أتممت المرحلة بنجاح، يمكنك الإنتقال إلى المرحلة التالية";
    l2.font = [UIFont fontWithName:@"TodaySHOP-Bold" size:25];
    l2.text = @"";

...
- (void)viewDidLoad
{
    ...
    startItem = (int)[self.tableArray count];
    IsLoading = NO;
    ...
    l1.text = @"You have";
    l2.text = @"successfully added your";
    l3.text = @"Score";
...
//                commaSeperatedFriends = [commaSeperatedFriends stringByAppendingFormat:@"%@,",[[friendInfo objectAtIndex:i] objectForKey:@"id"]];
            }
...
//        }];
        ...
        if([self.tableArray count] == 0) {
            [AlertMessage Display_Empty_error_message_WithLanguage:@"EN"];
            [self.navigationController popViewControllerAnimated:YES];
        }
...
-(UITableViewCell *) tableView: (UITableView *) tableView cellForRowAtIndexPath : (NSIndexPath *) indexPath {
    ...
    if(indexPath.row*4 < [self.tableArray count]) {
        cell.numberLabel1.text = [[NSString alloc] initWithFormat:@"Level %@", [[self.tableArray objectAtIndex:indexPath.row*4] objectForKey:@"level"]];
    ...
    if(indexPath.row*4 + 1 < [self.tableArray count]) {
        cell.numberLabel2.text = [[NSString alloc] initWithFormat:@"Level %@", [[self.tableArray objectAtIndex:indexPath.row*4 + 1] objectForKey:@"level"]];
    ...
    if(indexPath.row*4 + 2 < [self.tableArray count]) {
        cell.numberLabel3.text = [[NSString alloc] initWithFormat:@"Level %@", [[self.tableArray objectAtIndex:indexPath.row*4 + 2] objectForKey:@"level"]];
    ...
    if(indexPath.row*4 + 3 < [self.tableArray count]) {
        cell.numberLabel4.text = [[NSString alloc] initWithFormat:@"Level %@", [[self.tableArray objectAtIndex:indexPath.row*4 + 3] objectForKey:@"level"]];

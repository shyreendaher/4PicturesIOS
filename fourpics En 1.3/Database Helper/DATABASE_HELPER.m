...
+ (void) store_questions : (NSMutableArray *) questions {
    ...
    NSString *databasePath = [documentsDir stringByAppendingPathComponent:[self getDatabaseName]];
...
+ (NSMutableArray *) read_questions_for_levels : (int) level {
	...
	NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
	NSString *documentsDir = [paths objectAtIndex:0];
	NSString *databasePath = [documentsDir stringByAppendingPathComponent:@"questionsNew.sqlite"];
    
    NSMutableArray *questionsArray = [[NSMutableArray alloc] init];
    
    for (int i = level; i<13; i++) {
        ...
        if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
            ...
            if(sqlite3_prepare_v2(database, sql, -1, &read_statement, NULL) == SQLITE_OK)
            {
                ...
                sqlite3_bind_text(read_statement, 1, [[[NSString alloc] initWithFormat:@"%d", i] UTF8String], -1, SQLITE_TRANSIENT);
...
+ (void) delete_questions_for_level : (int) level {
...
//            sqlite3_bind_text(add_statement, 1, [[[NSString alloc] initWithFormat:@"%d", question.QId] UTF8String], -1, SQLITE_TRANSIENT);
//            sqlite3_bind_text(add_statement, 2, [question.QLevelId UTF8String], -1, SQLITE_TRANSIENT);
//            sqlite3_bind_text(add_statement, 3, [question.QAnswer UTF8String], -1, SQLITE_TRANSIENT);
//            sqlite3_bind_text(add_statement, 4, [question.QHint UTF8String], -1, SQLITE_TRANSIENT);
//            sqlite3_bind_text(add_statement, 5, [question.QCategory UTF8String], -1, SQLITE_TRANSIENT);
//            sqlite3_bind_text(add_statement, 6, [[[NSString alloc] initWithFormat:@"%d", question.IsCharactersRemoved] UTF8String], -1, SQLITE_TRANSIENT);
//            sqlite3_bind_text(add_statement, 7, [[[NSString alloc] initWithFormat:@"%d", question.IsHintShown] UTF8String], -1, SQLITE_TRANSIENT);
//            sqlite3_bind_text(add_statement, 8, [[[NSString alloc] initWithFormat:@"%d", question.IsAnswered] UTF8String], -1, SQLITE_TRANSIENT);
...
+ (void) store_questions_from_new_db : (NSMutableArray *) questions {
...
+ (BOOL) update_questions : (NSMutableArray *) fixesArr {
    ...
    for (int i =0; i < [fixesArr count]; i++) {
        ...
        if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
            const char * sql = "UPDATE TABLE_QUESTIONS SET QAnswer =? , QHint=? , QCategory=? WHERE QId=?";
            sqlite3_stmt *update_statement = nil;
            ...
            if(sqlite3_prepare_v2(database, sql, -1, &update_statement, NULL) == SQLITE_OK)
            {
                ...
                sqlite3_bind_text(update_statement, 3, [[dict objectForKey:@"category"] UTF8String], -1, SQLITE_TRANSIENT);

                sqlite3_bind_text(update_statement, 4, [[[NSString alloc] initWithData:[[NSData alloc] initWithBase64EncodedString:[dict objectForKey:@"questionId"]] encoding:NSUTF8StringEncoding] UTF8String], -1, SQLITE_TRANSIENT);
            }
...
+ (BOOL) add_questions_till_level12 : (int) fromIndex{
    ...
    levelsArray = [self read_questions_for_levels:fromIndex];

    dispatch_group_t d_group = dispatch_group_create();
    dispatch_queue_t bg_queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    
    dispatch_group_async(d_group, bg_queue, ^{
        [self store_questions_from_new_db:levelsArray];
    });
    ...
    dispatch_group_notify(d_group, dispatch_get_main_queue(), ^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"applyupdates" object:nil];
...
+ (NSMutableArray *) read_questions_for_reload : (int) level {
...
+ (int) reload_questions_for_level : (int) level {
    NSMutableArray * levelsArray = [[NSMutableArray alloc] init];
    levelsArray = [self read_questions_for_reload:level];
    ...
    dispatch_group_notify(d_group, dispatch_get_main_queue(), ^{
        dispatch_release(d_group);
        NSLog(@"All background tasks are done!!");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ReloadLevelCompleted" object:nil];

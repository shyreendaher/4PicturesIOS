         ...
         ^(FBRequestConnection *connection,
           ...
           NSError *error) {
             if (!error) {
                 ...
                 [AlertMessage Display_Facebook_error_message_WithLanguage:@"Ar"];
             }
         }];
    }
...
- (void) parse_question_XMLData:(NSString *)XmlString {
    ...
    @try {
        ...
        if ([appDelegate.currentLevelQuestionArray count] != 0){
            [self get_question_for_next_level];
            MovieViewController *instanse = [[MovieViewController alloc] init];
            [self.navigationController pushViewController:instanse animated:YES];
            [instanse release];
        }else {
            [AlertMessage Display_empty_level_error_message_WithLanguage:@"Ar"];
        }

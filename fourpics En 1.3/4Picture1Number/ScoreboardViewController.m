...
//                 NSLog(@"ME: %@", user);
                 NSLog(@"test");
                 NSLog(@"usr_id::%@",[user objectForKey:@"id"]);
...
- (void) post_scoreboard_list_with_facebookID :(NSString*) userID abd_usernamer :(NSString*) username {
    ...
    linkNumber = 2;    
    NSString *post = [[NSString alloc] initWithFormat:@"secret=5GhzqyJnaQuiVbzb9oPlk1QavbN6YfazBdH6tEaNbp0gDg5EqvCh5aLp67FbdAz5HfaYtcdB7YihNg&facebookId=%@&name=%@&score=%@&rand=%f", userID, username, [Cryptography TripleDES:[SimpleKeychain load:@"ScoreEn"] algo:kCCDecrypt key:@"Sfg$93@B"],[NSDate timeIntervalSinceReferenceDate]];
    NSData *postData = [post dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
    NSString *postLength =  [[NSString alloc] initWithFormat:@"%lu", (unsigned long)[postData length]];
